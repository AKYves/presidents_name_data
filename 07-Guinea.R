...
webpage <- read_webpage("Guinea")
vectors_ranges <- 60:71

presidents_list <- extract_presidents(webpage, token_word_two, vectors_ranges)
presidents_list[10] <- str_remove(presidents_list[10], "\\)\\s\\([^\\(]+\\)\\s+")
presidents_list[6] <- str_remove(presidents_list[6], "^(\\([^\\(]+\\)\\s+)")
presidents_list[4] <- str_remove(presidents_list[4], "^([^\\)]+\\)\\s+)")
presidents_list[8] <- str_remove(presidents_list[8], "^([^\\)]+\\)\\s+)")

presidents_list[10] <- paste(presidents_list[10], presidents_list[11])

country_data <- data.frame(country = "Guinea", presidents = presidents_list)
...
write.csv(country_data, file = "output_data/Guinea.csv",
          row.names = FALSE)
