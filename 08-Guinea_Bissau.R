...
webpage <- read_webpage("Guinea Bissau")
vectors_ranges <- 136:181

presidents_list <- extract_presidents(webpage, token_word_one, vectors_ranges)

country_data <- data.frame(country = "Guinea Bissau", presidents = presidents_list)
...
write.csv(country_data, file = "output_data/Guinea_Bissau.csv",
          row.names = FALSE)
